...
export function parseDoorDashEarnings(csvText: string): ParseResult<DoorDashEarningsRow> {
  ...
  dataRows.forEach((cells, i) => {
    try {
      ...
      const totalPay   = totalPayRaw != null
        ? parseMoney(totalPayRaw)
        : basePay + tip + peakPay;
...
export function parseDoorDashOrders(csvText: string): ParseResult<DoorDashOrderRow> {
  ...
  dataRows.forEach((cells, i) => {
    try {
      ...
      const pickupAddress  = get('merchant address') ?? get('pickup address') ?? get('store address') ?? '';
      const dropoffAddress = get('delivery address') ?? get('dropoff address') ?? get('customer address') ?? '';
      ...
      const durationMinutes = durationRaw != null
        ? (parseFloat(durationRaw.replace(/[^\d.]/g, '')) || null)
        : null;
